...
class PointsService
{
    public function earn(Membership $membership, int $points, ?Visit $visit = null, ?string $description = null): PointTransaction
    {
        if ($points <= 0) {
            throw new InvalidArgumentException('Points to earn must be greater than zero.');
        }

        return DB::transaction(function () use ($membership, $points, $visit, $description) {
            ...
            return PointTransaction::create([
                ...
                'description' => $description ?? 'Points earned from visit',
            ]);
        });
    }

    public function redeem(Membership $membership, int $points, ?string $description = null): PointTransaction
    {
        if ($points <= 0) {
            throw new InvalidArgumentException('Points to redeem must be greater than zero.');
        }

        return DB::transaction(function () use ($membership, $points, $description) {
            $membership = Membership::query()->lockForUpdate()->findOrFail($membership->id);
