@php
    ...
    $dial = \App\Support\Countries::dial($business->country);
    $hotlineRaw = old('hotline_local', '');
    if ($hotlineRaw === '' && $business->hotline) {
        $hotlineRaw = preg_replace('/^\+?'.preg_quote(ltrim($dial, '+'), '/').'\s*/', '', $business->hotline) ?: $business->hotline;
    }
@endphp
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-end justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <x-settings-back />
                @unless ($editing)
                    <a href="{{ route('business.edit', ['edit' => 1]) }}" class="loop-btn-mint !py-2.5">{{ __('loop.edit') }}</a>
                @endunless
            </div>
        </div>
    </x-slot>
    ...
    @unless ($editing)
        <div class="mx-auto max-w-xl space-y-5">
            <section class="rounded-[1.75rem] border border-ink/8 bg-white/90 p-6 sm:p-7">
                <div class="flex items-center gap-4">
                    @if ($business->logoUrl())
                        <img src="{{ $business->logoUrl() }}" alt="" class="h-16 w-16 rounded-2xl object-cover">
                    @else
                        <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-ink font-display text-xl text-lime">{{ mb_substr($business->name, 0, 1) }}</div>
                    @endif
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">{{ __('loop.business_logo') }}</p>
                        <p class="mt-1 font-display text-xl font-semibold">{{ $business->name }}</p>
                        <p class="mt-1 text-sm text-ink-muted">{{ $business->city }} · {{ $business->currency }}</p>
                    </div>
                </div>
                <dl class="mt-6 space-y-4 text-sm">
                    <div class="flex justify-between gap-4 border-b border-ink/5 pb-3">
                        <dt class="text-ink-muted">{{ __('loop.status') }}</dt>
                        <dd class="font-semibold">{{ $business->is_active ? __('loop.active') : __('loop.inactive') }}</dd>
                    </div>
                    <div class="flex justify-between gap-4 border-b border-ink/5 pb-3">
                        <dt class="text-ink-muted">{{ __('loop.hotline') }}</dt>
                        <dd class="font-semibold">{{ $business->hotline ?: '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4">
                        <dt class="text-ink-muted">{{ __('loop.description') }}</dt>
                        <dd class="max-w-[60%] text-right font-semibold">{{ $business->description ?: '—' }}</dd>
                    </div>
                </dl>
            </section>
        </div>
    @else
        <form method="POST" action="{{ route('business.update') }}" enctype="multipart/form-data" class="loop-panel mx-auto max-w-xl space-y-5 p-6 sm:p-8" autocomplete="off">
            @csrf
            @method('PATCH')
                ...
                <div class="grid grid-cols-[7.5rem_1fr] gap-2">
                    <select name="hotline_country_code" class="loop-input">
                        @foreach ($countries as $code => $meta)
                            <option value="{{ $meta['dial'] }}" @selected(old('hotline_country_code', $dial) === $meta['dial'])>{{ $meta['flag'] }} {{ $meta['dial'] }}</option>
                        @endforeach
                    </select>
                    <input name="hotline_local" value="{{ $hotlineRaw }}" class="loop-input" inputmode="tel" placeholder="712 345 678">
                </div>
            ...
            <section class="space-y-3 rounded-2xl border border-ink/10 bg-chalk/40 p-4"
                ...
                <div x-show="enabled" class="grid gap-3 sm:grid-cols-3" :class="enabled ? '' : 'hidden'">
            ...
            <div class="flex gap-3">
                <a href="{{ route('business.edit') }}" class="loop-btn-ghost flex-1 text-center">{{ __('loop.back') }}</a>
                <button class="loop-btn-mint flex-1">{{ __('loop.save_changes') }}</button>
            </div>
        </form>
    @endunless
</x-app-layout>
