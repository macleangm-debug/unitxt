...
<div class="{{ $compact ? 'mt-8' : 'mt-10' }} grid gap-4 sm:grid-cols-2 {{ $plans->count() >= 4 ? 'xl:grid-cols-4' : 'lg:grid-cols-3' }}">
    @foreach ($plans as $plan)
        ...
        <div @class([
            ...
            'border-violet bg-violet-soft/40 ring-2 ring-violet' => $isHighlight,
            'border-ink/10 bg-white hover:-translate-y-0.5' => ! $isHighlight,
        ])>
            @if ($isHighlight)
                <p class="mb-3 inline-flex w-fit rounded-full bg-ink px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-lime">{{ __('loop.most_popular') }}</p>
            @endif
            ...
            @if ($plan->key === 'free')
                <a href="{{ $ctaRoute }}" class="loop-btn mt-6 w-full">{{ __('loop.start_trial') }}</a>
            @else
                <a href="{{ $ctaRoute }}{{ str_contains($ctaRoute, '?') ? '&' : '?' }}plan={{ $plan->key }}" @class([
                    'mt-6 inline-flex w-full justify-center rounded-2xl px-4 py-3.5 text-sm font-semibold transition',
