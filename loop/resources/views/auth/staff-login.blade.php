...
>
    <form method="POST" action="{{ route('staff.login') }}" class="space-y-4" autocomplete="off" x-data="{
        ...
        {{-- Decoy fields reduce browser "save password" prompts --}}
        <input type="text" name="username" value="" autocomplete="username" tabindex="-1" aria-hidden="true" class="pointer-events-none absolute h-0 w-0 opacity-0" style="position:absolute;left:-9999px">
        <input type="password" name="password_fake" value="" autocomplete="new-password" tabindex="-1" aria-hidden="true" class="pointer-events-none absolute h-0 w-0 opacity-0" style="position:absolute;left:-9999px">
        ...
        <div>
            ...
            <div class="mt-1 flex overflow-hidden rounded-2xl border border-ink/10 bg-white shadow-sm focus-within:border-violet focus-within:ring-1 focus-within:ring-violet">
                <select name="country_code" x-model="country" class="shrink-0 border-0 border-r border-ink/10 bg-chalk py-3 pl-3 pr-8 text-sm font-semibold focus:ring-0" autocomplete="off">
                ...
                <input name="phone" value="{{ old('phone', $isAdmin ? '710000000' : '') }}" class="min-w-0 flex-1 border-0 bg-transparent px-3 py-3 text-base tracking-wide focus:ring-0" placeholder="7xxxxxxxx" required autofocus inputmode="tel" autocomplete="off">
            </div>
        </div>
        <div>
            <label class="loop-label">{{ __('loop.password') }}</label>
            <input type="password" name="password" class="loop-input" required value="{{ $isAdmin ? 'password' : '' }}" autocomplete="new-password">
        </div>
