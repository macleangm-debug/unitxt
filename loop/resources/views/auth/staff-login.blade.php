    ...
    <form method="POST" action="{{ route('staff.login') }}" class="space-y-4" autocomplete="off" data-lpignore="true" data-1p-ignore="true" x-data="{
        ...
        <div>
            ...
            <div class="mt-1 flex overflow-hidden rounded-2xl border border-ink/10 bg-white shadow-sm focus-within:border-violet focus-within:ring-1 focus-within:ring-violet">
                ...
                <input name="phone" value="{{ old('phone', $isAdmin ? '710000000' : '') }}" class="min-w-0 flex-1 border-0 bg-transparent px-3 py-3 text-base tracking-wide focus:ring-0" placeholder="7xxxxxxxx" required autofocus inputmode="tel" autocomplete="off" data-lpignore="true" data-1p-ignore="true">
            </div>
        </div>
        <div>
            <label class="loop-label">{{ __('loop.password') }}</label>
            <input type="password" name="password" class="loop-input" required value="{{ old('password') }}" autocomplete="off" data-lpignore="true" data-1p-ignore="true">
        </div>
