<x-app-layout>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <a href="{{ route('business.edit') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(40)" :class="{ 'is-shown': shown }">
        ...
        <a href="{{ route('shops.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(80)" :class="{ 'is-shown': shown }">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.shops') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold" x-data="loopCountUp({{ (int) $shopCount }})" x-text="formatted()">{{ $shopCount }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_shops') }}</p>
        </a>
        <a href="{{ route('campaigns.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(120)" :class="{ 'is-shown': shown }">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.campaigns_and_offers') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold" x-data="loopCountUp({{ (int) ($campaignCount + $offerCount) }})" x-text="formatted()">{{ $campaignCount + $offerCount }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_loyalty') }}</p>
        </a>
        <a href="{{ route('staff.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(160)" :class="{ 'is-shown': shown }">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.staff') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold" x-data="loopCountUp({{ (int) $staffCount }})" x-text="formatted()">{{ $staffCount }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_staff') }}</p>
        </a>
        <a href="{{ route('raffles.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(200)" :class="{ 'is-shown': shown }">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.raffles') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold" x-data="loopCountUp({{ (int) $raffleCount }})" x-text="formatted()">{{ $raffleCount }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_raffles') }}</p>
        </a>
        <a href="{{ route('content-studio.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(240)" :class="{ 'is-shown': shown }">
        ...
        <a href="{{ route('settings.referrals') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(280)" :class="{ 'is-shown': shown }">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.referrals') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold" x-data="loopCountUp({{ (int) $referralCount }})" x-text="formatted()">{{ $referralCount }}</p>
        ...
        <a href="{{ route('billing.show') }}" class="loop-panel loop-panel--energy group relative block overflow-hidden p-6 transition hover:-translate-y-0.5 loop-reveal" x-data="loopReveal(320)" :class="{ 'is-shown': shown }">
