<x-app-layout>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <a href="{{ route('shops.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            ...
            <p class="relative mt-3 font-display text-3xl font-semibold" >{{ number_format((int) $shopCount) }}</p>
        ...
        <a href="{{ route('campaigns.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            ...
            <p class="relative mt-3 font-display text-3xl font-semibold" >{{ number_format((int) ($campaignCount + $offerCount)) }}</p>
        ...
        <a href="{{ route('staff.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            ...
            <p class="relative mt-3 font-display text-3xl font-semibold" >{{ number_format((int) $staffCount) }}</p>
        ...
        @endif
        @if (! empty($gamesVisible))
        <a href="{{ route('games.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            <p class="relative text-xs font-semibold uppercase tracking-[0.14em] text-violet">{{ __('loop.games_wins') }}</p>
            <p class="relative mt-3 font-display text-3xl font-semibold">{{ number_format((int) ($gameCount ?? 0)) }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_games') }}</p>
        </a>
        @endif
        @if (! empty($rafflesEnabled) || $raffleCount > 0)
        <a href="{{ route('raffles.index') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            ...
            <p class="relative mt-3 font-display text-3xl font-semibold" >{{ number_format((int) $raffleCount) }}</p>
            <p class="relative mt-2 text-sm text-ink-muted">{{ __('loop.settings_raffles') }}</p>
        </a>
        @endif
        @if (! empty($studioEnabled))
        ...
        <a href="{{ route('settings.referrals') }}" class="loop-panel loop-panel--energy group block p-6 transition hover:-translate-y-0.5">
            ...
            <p class="relative mt-3 font-display text-3xl font-semibold" >{{ number_format((int) $referralCount) }}</p>
