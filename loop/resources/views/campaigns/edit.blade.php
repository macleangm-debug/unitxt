...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                <h1 class="font-display text-2xl font-semibold sm:text-3xl">{{ __('loop.edit') }} · {{ $campaign->displayName() }}</h1>
                <p class="mt-1 text-sm text-ink-muted">{{ __('loop.edit_campaign_simple_blurb') }}</p>
            </div>
            ...
            <div data-step="2" x-show="step === 2" x-cloak class="space-y-4">
                ...
                @else
                    ...
                    <div>
                        ...
                        <input type="number" name="bonus_points" class="loop-input" value="{{ old('bonus_points', $campaign->bonus_points) }}" data-required="1">
                    </div>
                @endif
