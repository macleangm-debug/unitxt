        ...
        <div class="flex h-16 items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                @if (Auth::user()->isCustomer())
                    <a href="{{ route('discover') }}" class="hidden text-sm font-semibold text-ink-muted hover:text-ink sm:inline">{{ __('loop.discover') }}</a>
                    <a href="{{ route('memberships.index') }}" class="hidden text-sm font-semibold text-ink-muted hover:text-ink sm:inline">{{ __('loop.wallets') }}</a>
                @endif
