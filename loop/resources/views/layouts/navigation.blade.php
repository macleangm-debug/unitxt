...
@if (Auth::user()->isCustomer())
    <nav class="loop-bottom-nav md:hidden" aria-label="{{ __('loop.home') }}">
        <div class="mx-auto grid max-w-lg grid-cols-3 px-2 py-2 text-center text-[11px] font-semibold">
            <a href="{{ route('dashboard') }}" @class(['flex flex-col items-center gap-1 rounded-xl px-2 py-2', request()->routeIs('dashboard') ? 'text-violet' : 'text-ink-muted'])>
                <span class="text-base leading-none">⌂</span>
                {{ __('loop.home') }}
            </a>
            <a href="{{ route('memberships.index') }}" @class(['flex flex-col items-center gap-1 rounded-xl px-2 py-2', request()->routeIs('memberships.*') ? 'text-violet' : 'text-ink-muted'])>
                <span class="text-base leading-none">◇</span>
                {{ __('loop.wallets') }}
            </a>
            <a href="{{ route('discover') }}" @class(['flex flex-col items-center gap-1 rounded-xl px-2 py-2', request()->routeIs('discover*') ? 'text-violet' : 'text-ink-muted'])>
                <span class="text-base leading-none">◎</span>
                {{ __('loop.discover') }}
            </a>
        </div>
