...
<div class="min-h-screen md:grid md:grid-cols-2">
    <aside class="relative hidden overflow-hidden bg-ink md:flex md:flex-col md:justify-between md:p-10 lg:p-12">
        ...
        <div class="relative">
            ...
            <div class="mt-12 inline-flex rotate-[-6deg] items-center gap-2 rounded-2xl border border-lime/40 bg-white/5 px-4 py-3 text-lime">
                <span class="font-display text-sm font-semibold uppercase tracking-[0.2em]">{{ $asideStamp ?: __('loop.customer_stamp') }}</span>
            </div>
            <p class="mt-8 font-display text-3xl font-semibold leading-tight text-white lg:text-4xl">{{ $asideTitle ?: __('loop.auth_aside_title') }}</p>
        ...
        <div class="relative mt-10 space-y-3 text-sm text-white/65">
            <p><span class="text-lime">◆</span> {{ $asidePoint1 ?: __('loop.auth_aside_1') }}</p>
            <p><span class="text-lime">◆</span> {{ $asidePoint2 ?: __('loop.auth_aside_2') }}</p>
            <p><span class="text-lime">◆</span> {{ $asidePoint3 ?: __('loop.auth_aside_3') }}</p>
        </div>
    </aside>

    <div class="relative flex min-h-screen flex-col bg-chalk">
        <div class="pointer-events-none absolute inset-0">
            <div class="absolute -left-16 top-10 h-64 w-64 rounded-full bg-violet/10 blur-3xl"></div>
            <div class="absolute right-0 top-0 h-72 w-72 rounded-full bg-lime/15 blur-3xl"></div>
        </div>
        <div class="relative flex items-center justify-between gap-3 px-4 py-4 sm:px-8">
