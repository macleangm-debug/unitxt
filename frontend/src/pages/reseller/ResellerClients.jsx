...
export default function ResellerClients() {
  ...
  const transfer = async (e) => {
    ...
    try {
      ...
      toast.success(`Transferred ${creditsShort(amount)} credits to ${open.name}`);
      setOpen(null); setAmount(1000); setNote(""); load();
    } catch(err){ toast.error(fmtErr(err.response?.data?.detail)); }
  };

  return (
    <div>
      <PageHeader overline="Distribution" title="Clients" desc="Onboard clients via your reseller code, fund their credits, manage their access."/>
      <Table testid="r-clients-table" rows={items} empty="No clients yet." columns={[
        ...
        { key:"wallet_balance", label:"Credits", mono:true, render: r => creditsShort(r.wallet_balance) },
        ...
        {open && (
          <form onSubmit={transfer} className="space-y-3">
            <div className="border border-zinc-800 bg-zinc-950 p-3 text-xs">
              <div className="flex justify-between"><span className="text-zinc-500">Client credits</span><span className="font-mono">{creditsShort(open.wallet_balance)}</span></div>
            ...
            <Field label="Credits to transfer"><Input type="number" min={1} step="1" value={amount} onChange={(e)=>setAmount(e.target.value)} required data-testid="transfer-amount"/></Field>
            <Field label="Note (optional)"><Input value={note} onChange={(e)=>setNote(e.target.value)} data-testid="transfer-note"/></Field>
            <Btn type="submit" className="w-full" data-testid="transfer-submit">Send credits</Btn>
          </form>
        )}
      </Modal>
    </div>
