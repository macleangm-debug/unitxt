...
export default function AdminWallets() {
  ...
  return (
    <div>
      <PageHeader overline="Treasury" title="All wallets" desc="Liabilities held on the platform."/>
      <div className="grid gap-4 md:grid-cols-3">
        <Stat label="Total liabilities" value={money(total)} accent="orange"/>
        <Stat label="Wallet count" value={items.length}/>
        <Stat label="Average balance" value={money(items.length ? total/items.length : 0)}/>
      </div>
      <Table testid="wallets-table" rows={items} columns={[
        ...
        { key:"balance", label:"Balance", mono:true, render: r => <span className={r.balance>0?"text-emerald-400":""}>{money(r.balance)}</span> },
      ]}/>
    </div>
  );
}
